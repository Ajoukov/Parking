...
struct LoginView: View {
    ...
    private let apiURL = "http://localhost:\(Bundle.main.infoDictionary?["REACT_APP_BPORT"] as? String ?? "5001")/api/login"
    ...
    var body: some View {
        ...
        .alert(isPresented: $alertActive) {
            Alert(
                ...
                message: Text(loginSuccess ? "Login Successful" : errorMessage),
    ...
    private func login() {
        ...
        URLSession.shared.dataTask(with: request) { data, response, error in
            guard let data = data, error == nil else {
                DispatchQueue.main.async {
                    errorMessage = error?.localizedDescription ?? "Error logging in"
                    alertActive = true
                    loginSuccess = false
                }
                return
            ...
            do {
                if let jsonResponse = try JSONSerialization.jsonObject(with: data, options: []) as? [String: Any],
                   let userDict = jsonResponse["user"] as? [String: Any] {
                    DispatchQueue.main.async {
                        // Accessing specific fields from the user dictionary
                        user.user = User(
